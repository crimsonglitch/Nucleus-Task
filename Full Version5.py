...
class DatabaseManager:
    ...
    def create_default_users(self):
        default_users = [
            ...
            ("ceo1", "ceo123", "Chief Executive Officer", "ceo",
             "Management", 200000, "ceo@company.com"),
...
class HRManagementSystem(QMainWindow):
    def __init__(self):
        ...
        # System configuration - CEO eklendi
        self.roles = ["root", "ceo", "boss", "admin", "moderator",
    ...
    def check_permission(self, permission):
        ...
        role_permissions = {
            'root': ['all'],
            'ceo': ['manage_employees', 'view_reports', 'approve_leave', 'admin_access', 'terminate_employee', 'manage_recruitment'],
    ...
    def can_terminate_employee(self, target_role):
        ...
        try:
            ...
            if current_role not in ['root', 'ceo', 'boss', 'admin', 'moderator']:
                return False
            ...
            # CEO can terminate boss and below
            if current_role == 'ceo':
                return target_role_index >= self.roles.index('boss')
            
            # Boss can terminate admin and below
            if current_role == 'boss':
                return target_role_index >= self.roles.index('admin')
    ...
    def get_available_roles_for_creation(self):
        ...
        current_role = self.current_user['role']
        if current_role == "root":
            return self.roles[1:]  # Can create any role except root (ceo, boss, admin, ...)
        elif current_role == "ceo":
            return self.roles[2:]  # Can create boss and below (boss, admin, moderator, ...)
        elif current_role == "boss":
            return self.roles[3:]  # Can create admin and below (admin, moderator, senior, ...)
        elif current_role == "admin":
            return self.roles[4:]  # Can create moderator and below (moderator, senior, ...)
        elif current_role == "moderator":
            return self.roles[5:]  # Can create senior and below (senior, engineer, ...)
        else:
    ...
    def get_available_departments_for_role(self, role):
        ...
        if role in ["root", "ceo", "boss"]:
            return ["Management"]  # Only Management for root, CEO and boss
        else:
    ...
    def can_modify_employee(self, target_role):
        ...
        current_role = self.current_user['role']
        try:
            current_role_index = self.roles.index(current_role)
            target_role_index = self.roles.index(target_role)
            
            # Root can modify everyone except other roots
            if current_role == "root":
                return target_role != "root"
            
            # CEO can modify boss and below
            if current_role == "ceo":
                return target_role_index >= self.roles.index('boss')
            
            # Boss can modify admin and below
            if current_role == "boss":
                return target_role_index >= self.roles.index('admin')
            
            # Admin can modify moderator and below
            if current_role == "admin":
                return target_role_index >= self.roles.index('moderator')
            
            # Moderator can modify senior and below
            if current_role == "moderator":
                return target_role_index >= self.roles.index('senior')
    ...
    def show_edit_employee_dialog(self):
        ...
        # Check permissions - both for modification and role assignment
        ...
        self.edit_emp_role.addItems(available_roles)
        
        # If current employee role is not in available roles, add it but disable it
        if emp_data[3] not in available_roles:
            self.edit_emp_role.addItem(emp_data[3])
            self.edit_emp_role.setCurrentText(emp_data[3])
            # Make the combo box read-only if user can't change the role
            self.edit_emp_role.setEnabled(False)
        else:
            self.edit_emp_role.setCurrentText(emp_data[3])
    ...
    def save_employee_changes(self, dialog):
        # Role permission check
        selected_role = self.edit_emp_role.currentText()
        available_roles = self.get_available_roles_for_creation()
        if selected_role not in available_roles and self.edit_emp_role.isEnabled():
            QMessageBox.warning(self, "Permission Denied",
                               f"You don't have permission to assign {selected_role} role")
            return
    ...
    def refresh_employee_list(self):
        ...
        # Yetki seviyesine göre personel listesini filtrele
        current_role = self.current_user['role']
        
        if current_role == "root":
            # Root herkesi görebilir
            query = """
            ...
            # Diğer roller sadece kendi seviyesinden aşağıdakileri görebilir
            try:
                current_role_index = self.roles.index(current_role)
                allowed_roles = self.roles[current_role_index + 1:]  # Kendi seviyesinden aşağıdakiler
                ...
                # Rol bulunamadı, hiç kimseyi göremez
                query = """
                SELECT id, name, role, department, salary,
                       CASE WHEN is_active THEN 'Active' ELSE 'Inactive' END as status
                FROM employees
    ...
    def filter_employees(self):
        ...
        # Yetki seviyesine göre temel sorgu
        current_role = self.current_user['role']
        ...
        if current_role == "root":
            ...
            try:
                current_role_index = self.roles.index(current_role)
                allowed_roles = self.roles[current_role_index + 1:]
                ...
                if allowed_roles:
                    ...
                    SELECT id, name, role, department, salary,
                           CASE WHEN is_active THEN 'Active' ELSE 'Inactive' END as status
                    FROM employees
                    WHERE role IN ({placeholders}) AND LOWER(name) LIKE ?
                    """
                    params = allowed_roles + [f"%{name_filter}%"]
                else:
                    base_query = """
                    SELECT id, name, role, department, salary,
                           CASE WHEN is_active THEN 'Active' ELSE 'Inactive' END as status
                    FROM employees
                    WHERE 1=0 AND LOWER(name) LIKE ?
                    """
                    params = [f"%{name_filter}%"]
            except ValueError:
                base_query = """
