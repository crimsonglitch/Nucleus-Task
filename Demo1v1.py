...
class DatabaseManager:
    ...
    def create_indexes(self):
        ...
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_leave_applications_employee_id ON leave_applications(employee_id)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_leave_applications_status ON leave_applications(status)")
        self.connection.commit()

    def create_default_users(self):
        default_users = [
            ("root", "toor123!", "System Root", "root", "Management", 0, "root@company.com"),
        ...
        for user in default_users:
            ...
            if not cursor.fetchone():
                ...
                encrypted_contact = SecurityUtils.encrypt_data(user[6], "secure_master_key_placeholder")
                ...
                """, (
                    ...
                    user[4],
                    user[5],
                    encrypted_contact['encrypted_data']['triple_encrypted'],
                    encrypted_contact['encrypted_data'].get('iv', ''),
...
class HRManagementSystem(QMainWindow):
    def __init__(self):
        ...
        self.master_key = "secure_master_key_placeholder"  # In production, use proper key management
        ...
        self.leave_types = ["Annual", "Sick", "Maternity", "Paternity", "Unpaid", "Other"]
    ...
    def update_status_bar(self):
        ...
        self.statusBar().showMessage(f"{user_info} | System Status: Operational | {current_time}")
    ...
    def init_login_ui(self):
        ...
        # Password
        self.password_input = StyledLineEdit(placeholder="Enter your password", password=True)
    ...
    def login(self):
        ...
        if user_data:
            if not user_data[12]:  # is_active check
                QMessageBox.warning(self, "Account Disabled", "Your account is disabled. Please contact HR.")
                return
                
            # Verify password
            salt = base64.b64decode(user_data[3])
            iterations = user_data[4]
            hashed_password = SecurityUtils.hash_password(password, salt, iterations)['hashed']
            
            if hashed_password == user_data[2]:
                # Decrypt contact information
                encrypted_contact = {
                    'triple_encrypted': user_data[9],
                    'iv': user_data[10],
                    'salt': user_data[11]
                }
                try:
                    contact = SecurityUtils.decrypt_data(encrypted_contact, self.master_key)
                except:
                    contact = "Encrypted"
                ...
                # Log login activity
                self.db.log_activity(self.current_user["id"], "login", f"User logged in from {self.get_ip_address()}")
    ...
    def init_main_ui(self):
        ...
        # Header
        header = QWidget()
        header.setStyleSheet("background-color: #4CAF50; color: white; padding: 10px; border-radius: 4px;")
        header_layout = QHBoxLayout(header)
        
        user_info = QLabel(f"Welcome, {self.current_user['name']} ({self.current_user['role']})")
        user_info.setStyleSheet("font-size: 16px; font-weight: bold;")
        header_layout.addWidget(user_info)
        
        logout_button = StyledButton("Logout", color="#f44336", hover_color="#d32f2f", pressed_color="#b71c1c")
    ...
    def check_permission(self, permission):
        ...
        role_permissions = {
            'root': ['all'],
            'boss': ['manage_employees', 'view_reports', 'approve_leave', 'admin_access'],
            'admin': ['manage_employees', 'view_reports', 'manage_recruitment', 'admin_access'],
            'moderator': ['manage_recruitment', 'view_team_reports'],
    ...
    def setup_employee_management_tab(self):
        ...
        self.employee_filter_input = StyledLineEdit(placeholder="Search by name...")
        ...
        if self.check_permission("manage_employees"):
            ...
            delete_button = StyledButton("Delete Employee", color="#f44336", hover_color="#d32f2f", pressed_color="#b71c1c")
        ...
        self.employee_table.setHorizontalHeaderLabels(["ID", "Name", "Role", "Department", "Salary"])
    ...
    def show_add_employee_dialog(self):
        ...
        cancel_button = StyledButton("Cancel", color="#f44336", hover_color="#d32f2f", pressed_color="#b71c1c")
    ...
    def save_new_employee(self, dialog):
        ...
        try:
            ...
            QMessageBox.warning(self, "Validation Error", "Invalid salary amount")
            return
        
        # Check if username exists
        cursor = self.db.connection.cursor()
        cursor.execute("SELECT id FROM employees WHERE username=?", (self.new_emp_username.text(),))
        if cursor.fetchone():
            QMessageBox.warning(self, "Error", "Username already exists")
            return
        
        # Hash password and encrypt contact
        password_data = SecurityUtils.hash_password(self.new_emp_password.text())
        encrypted_contact = SecurityUtils.encrypt_data(self.new_emp_contact.text(), self.master_key)
        current_time = datetime.now().isoformat()
    ...
    def show_edit_employee_dialog(self):
        ...
        cursor = self.db.connection.cursor()
        cursor.execute("""
        SELECT id, username, name, role, department, salary, contact, is_active
        ...
        if not self.can_modify_employee(emp_data[3]):
            QMessageBox.warning(self, "Permission Denied", "You don't have permission to edit this employee")
            return
        ...
        self.edit_emp_department.addItems(self.departments)
    ...
    def save_employee_changes(self, dialog):
        ...
        # Encrypt contact
        encrypted_contact = SecurityUtils.encrypt_data(self.edit_emp_contact.text(), self.master_key)
    ...
    def delete_employee(self):
        ...
        if not self.can_modify_employee(emp_role):
            QMessageBox.warning(self, "Permission Denied", "You don't have permission to delete this employee")
            return
    ...
    def show_employee_details(self, row, col):
        ...
        cursor = self.db.connection.cursor()
        cursor.execute("""
        SELECT id, username, name, role, department, salary, contact, hire_date, last_login
        FROM employees WHERE id=?
        """, (emp_id,))
        emp_data = cursor.fetchone()
        ...
        details_dialog.setMinimumSize(400, 300)
        ...
        <table border='0' cellpadding='5'>
            ...
            <tr><td><b>Last Login:</b></td><td>{emp_data[8] if emp_data[8] else 'Never'}</td></tr>
        </table>
        """)
    ...
    def setup_personal_tab(self):
        ...
        self.personal_department.setCurrentText(self.current_user['department'])
    ...
    def save_personal_info(self):
        ...
        if not all([new_name, new_contact]):
            QMessageBox.warning(self, "Validation Error", "Name and contact information are required")
            return
        
        # Encrypt contact
        encrypted_contact = SecurityUtils.encrypt_data(new_contact, self.master_key)
        ...
        try:
            ...
            QMessageBox.information(self, "Success", "Personal information updated successfully")
        except Exception as e:
    ...
    def change_password(self):
        ...
        current_hashed = SecurityUtils.hash_password(current_pass, salt, iterations)['hashed']
    ...
    def setup_leave_management_tab(self):
        ...
        if self.check_permission("approve_leave"):
            ...
            reject_button = StyledButton("Reject Selected", color="#f44336", hover_color="#d32f2f", pressed_color="#b71c1c")
    ...
    def show_new_leave_dialog(self):
        ...
        submit_button.clicked.connect(lambda: self.submit_leave_application(dialog))
    ...
    def submit_leave_application(self, dialog):
        ...
        if not reason:
            QMessageBox.warning(self, "Validation Error", "Please provide a reason for your leave")
            return
        ...
        try:
            ...
            self.notify_manager_about_leave(leave_type, days, start_date, end_date)
            
            QMessageBox.information(self, "Success", "Leave application submitted successfully")
            self.refresh_leave_list()
            dialog.accept()
        except Exception as e:
            QMessageBox.critical(self, "Error", f"Failed to submit leave application: {str(e)}")
    ...
    def refresh_leave_list(self):
        ...
        for leave in leaves:
            ...
            if leave[4] == "Approved":
                for col in range(6):
                    self.leave_table.item(row, col).setBackground(QColor(220, 255, 220))
            elif leave[4] == "Rejected":
                for col in range(6):
                    self.leave_table.item(row, col).setBackground(QColor(255, 220, 220))
    
    def approve_leave(self):
        if not self.check_permission("approve_leave"):
            QMessageBox.warning(self, "Permission Denied", "You don't have permission to approve leave")
            return
        
        selected = self.leave_table.selectedItems()
        if not selected:
            QMessageBox.warning(self, "Warning", "Please select a leave request to approve")
            return
        ...
        try:
            ...
            QMessageBox.critical(self, "Error", f"Failed to approve leave: {str(e)}")
    
    def reject_leave(self):
        if not self.check_permission("approve_leave"):
            QMessageBox.warning(self, "Permission Denied", "You don't have permission to reject leave")
            return
        
        selected = self.leave_table.selectedItems()
        if not selected:
            QMessageBox.warning(self, "Warning", "Please select a leave request to reject")
            return
        ...
        try:
            ...
            QMessageBox.critical(self, "Error", f"Failed to reject leave: {str(e)}")
    ...
    def setup_recruitment_tab(self):
        ...
        delete_candidate_button = StyledButton("Delete Candidate", color="#f44336", hover_color="#d32f2f", pressed_color="#b71c1c")
    ...
    def show_new_candidate_dialog(self):
        ...
        professional_layout.addRow("Position Applied:", self.candidate_position)
        professional_layout.addRow("Years of Experience:", self.candidate_experience)
    ...
    def save_candidate(self, dialog):
        ...
        if not all([
            ...
            QMessageBox.warning(self, "Validation Error", "Name, email and position are required")
            return
        ...
        # Generate filename
        safe_name = "".join(c for c in candidate_data["personal_info"]["name"] if c.isalnum() or c in " _-")
        ...
        try:
            ...
            QMessageBox.information(self, "Success", "Candidate saved successfully")
            self.refresh_candidate_list()
            dialog.accept()
        except Exception as e:
            ...
            if filename.endswith(".json"):
                try:
                    with open(os.path.join("candidates", filename), 'r', encoding='utf-8') as f:
                        ...
                        date = datetime.fromisoformat(data["created_at"]).strftime("%Y-%m-%d")
                        self.candidate_list.addItem(f"{name} | {position} | {status} | {date} | {filename}")
                except:
                    continue
    ...
    def view_candidate_details(self):
        ...
        if not selected:
            QMessageBox.warning(self, "Warning", "Please select a candidate to view")
            return
        ...
        try:
            ...
            dialog = QDialog(self)
            dialog.setWindowTitle(f"Candidate: {candidate_data['personal_info']['name']}")
            dialog.setMinimumSize(800, 600)
            ...
            personal_layout.addRow("Name:", QLabel(candidate_data["personal_info"]["name"]))
            personal_layout.addRow("Email:", QLabel(candidate_data["personal_info"]["email"]))
            personal_layout.addRow("Phone:", QLabel(candidate_data["personal_info"]["phone"]))
            personal_layout.addRow("Address:", QLabel(candidate_data["personal_info"]["address"]))
            personal_group.setLayout(personal_layout)
            layout.addWidget(personal_group)
            ...
            professional_layout.addRow("Position:", QLabel(candidate_data["professional_info"]["position"]))
            professional_layout.addRow("Experience:", QLabel(f"{candidate_data['professional_info']['experience']} years"))
            ...
            education_layout.addRow("Degree:", QLabel(candidate_data["education_info"]["degree"]))
            education_layout.addRow("University:", QLabel(candidate_data["education_info"]["university"]))
            education_layout.addRow("Graduation Year:", QLabel(candidate_data["education_info"]["graduation"]))
            education_group.setLayout(education_layout)
            layout.addWidget(education_group)
            ...
            self.candidate_status.addItems(["New", "Reviewed", "Interview Scheduled", "Hired", "Rejected"])
            self.candidate_status.setCurrentText(candidate_data.get("status", "New"))
            ...
            if self.check_permission("manage_recruitment"):
                save_button = StyledButton("Save Changes")
                save_button.clicked.connect(lambda: self.save_candidate_changes(filename, dialog))
                button_box.addWidget(save_button)
            ...
            QMessageBox.critical(self, "Error", f"Failed to load candidate: {str(e)}")
    ...
    def save_candidate_changes(self, filename, dialog):
        ...
        try:
            ...
            QMessageBox.critical(self, "Error", f"Failed to update candidate: {str(e)}")
    ...
    def generate_report(self):
        ...
        if report_type == "Employee List by Department":
            ...
            for emp in employees:
                ...
                report_text += f"{emp[0]} ({emp[1]}) - ${emp[3]:,.2f} - Hired: {emp[4][:10]}\n"
    ...
    def setup_admin_tab(self):
        ...
        if self.current_user['role'] in ['root', 'admin']:
            ...
            if self.current_user['role'] == 'root':
                init_button = StyledButton("Initialize System", color="#f44336", hover_color="#d32f2f", pressed_color="#b71c1c")
    ...
    def check_for_updates(self):
        ...
        QMessageBox.information(self, "Check for Updates", "This feature would check for system updates in a real implementation")
